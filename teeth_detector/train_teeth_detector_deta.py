...
class ResizeLongestEdge(A.DualTransform):
    def __init__(self, max_size=1333, interpolation=cv2.INTER_LINEAR, always_apply=True, p=1.0):
        super(ResizeLongestEdge, self).__init__(always_apply=always_apply, p=p)
...
def get_transform(train, image_processor):
    # Get normalization params from image processor config.
    ...
    if train:
        return A.Compose(
            [
                ResizeLongestEdge(max_size=1333, interpolation=cv2.INTER_LINEAR),
...
def main(
    ...
    model_checkpoint: str = "jozhang97/deta-resnet-50",
    output_dir: str = "tooth_detector_deta50",
    batch_size: int = 1,
    learning_rate: float = 1e-5,
    num_epochs_stage1: int = 2,
    num_epochs_stage2: int = 10,
    ...
    train_dataset = ToothDetectionJsonDataset(
        root=train_dataset_path, transforms=get_transform(train=True, image_processor=image_processor)
    )

    val_dataset = ToothDetectionJsonDataset(
        root=val_dataset_path, transforms=get_transform(train=False, image_processor=image_processor)
    )
